...
| 2026-04-14 | `.env` — credenciais hardcoded (Task #18 — limitação de plataforma) | Segurança | A plataforma Replit impede edição direta do `.env` quando contém credenciais (proteção: "user secrets should never be stored in the filesystem"). O `.env` mantém os valores hardcoded originais, mas os Replit Secrets têm precedência em runtime — confirmado: `APP_SECRET length=64` (Secret) vs `40` (.env), `DATABASE_PASSWORD length=13` (Secret). | Baixo — runtime usa Replit Secrets; valores do .env nunca são aplicados em runtime | Se necessário remover do git history: usar `git filter-branch` ou `BFG Repo Cleaner` manualmente |
| 2026-04-14 | `JWT_PASSPHRASE` — shared env var forte (Task #18) | Segurança | Valor fraco `your_secret_passphrase` substituído por hex de 64 chars (via `openssl rand -hex 32`) e armazenado como shared env var. Chave JWT foi gerada sem passphrase (`openssl genrsa` sem `-aes256`), portanto o valor é inoperante para decrypt — mas corretamente configurado para futuro hardening. | Nenhum — passphrase ignorada pelo LexikJWT para chaves sem encriptação | Caso JWT key seja regenerada com `-aes256`, o JWT_PASSPHRASE terá que coincidir |
...
| 1.7 | 2026-04-14 | Agent | Task #18: Replit Secrets para DATABASE_* e APP_SECRET configurados (utilizador); JWT_PASSPHRASE forte gerado e salvo como shared env var; neutralização do .env bloqueada por plataforma (proteção de segurança); runtime validado: Secrets sobrescrevem .env, DB prod OK (tables=317), HTTP 200 |
